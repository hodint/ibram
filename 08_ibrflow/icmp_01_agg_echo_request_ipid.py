...
# Filename mask used
filename_mask = 'echo_request-ipid'
...
GROUP_COLS = ["ip_id", "ip_src", "icmp_type", "icmp_code", "payload_hex", "IBR_info"]
...
def build_reason(group_row: pd.Series) -> str:
    ...
    return "ip_id;ip_src;icmp_type;icmp_code;payload_hex;IBR_info"
...
def collect_input_files(input_paths):
    ...
    for input_path in input_paths:
        p = Path(input_path)
        if p.is_dir():
            # If it's a directory, get all CSV files matching pattern _1.csv or _1.csv.gz
            files = sorted([
                fp for fp in p.iterdir()
                if fp.is_file() and (fp.name.endswith("_1.csv") or fp.name.endswith("_1.csv.gz"))
            ])
            all_files.extend(files)
        elif p.is_file():
            # If it's a file, add it directly if it matches the pattern
            if p.name.endswith("_1.csv") or p.name.endswith("_1.csv.gz"):
                all_files.append(p)
