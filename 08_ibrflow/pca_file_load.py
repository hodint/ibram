...
def load_df_from_pcap_csv_gz(file_name, logger):
    """
...
def load_df_from_pcap_ndjson_gz(file_name, logger):
    """
    Load an NDJSON (newline-delimited JSON, possibly gzipped) file containing pcap data
    into a dataframe.

    Each line in the NDJSON file should be a valid JSON object representing a packet record.

    :param file_name: The path to the NDJSON file.
    :param logger: Logger object for logging information.
    :return: A dataframe with the loaded data.
    """
    logger.info(f"Loading NDJSON file {file_name}")
    ...
    if not records:
        logger.warning(f"No records found in file {file_name}")
        return pd.DataFrame()
    ...
    for col in df.columns:
        if col in coldef.col_type_csv_import:
            try:
                df[col] = df[col].astype(coldef.col_type_csv_import[col])
            except Exception as e:
                logger.warning(f"Could not convert column {col} to type {coldef.col_type_csv_import[col]}: {e}")

    # IBR_info should already be a dictionary, but convert it if it is a string
    if 'IBR_info' in df.columns and df['IBR_info'].dtype == 'object':
        try:
            # Check if the first non-null value is a string
            first_val = df['IBR_info'].dropna().iloc[0] if not df['IBR_info'].dropna().empty else None
            if isinstance(first_val, str):
                df['IBR_info'] = df['IBR_info'].apply(json.loads)
        except Exception as e:
            logger.warning(f"Could not convert IBR_info to dict: {e}")
    ...
    fecha_base = obtener_fecha_inferior(df)
    fecha_top = obtener_fecha_superior(df)
    logger.info(f"NDJSON data loaded from file {file_name}, with date range: {fecha_base} to {fecha_top}")
